...
print("🔧 Repairing Phase 2 - Step 6: Activity Inspector Drawer...")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
backup_dir = f"_backup_step6_repair_{timestamp}"
...
# ==============================================================================
# NEW: Completely rewritten activity_inspector.js with:
# - Multiple selector strategies (data-attr, class-based, DataTables-friendly)
# - MutationObserver to catch dynamically-added rows
# - Detailed console diagnostics
# - Fallback delegated click on the entire document
# ==============================================================================
...
print("  ✅ Rewrote static/activity_inspector.js (v2 with auto-detection)")


# ==============================================================================
# Verify Inspector CSS/JS are properly included in all templates
# ==============================================================================

def verify_and_inject_inspector(template_path):
    ...
    try:
        ...
        needs_update = False
        # Ensure CSS is loaded (non-deferred so styles apply immediately)
        if 'activity_inspector.css' not in content:
        ...
        # Ensure JS is loaded (must NOT be deferred so console log appears immediately)
        if 'activity_inspector.js' not in content:
            content = content.replace(
                "</head>",
                '    <script src="{{ url_for(\'static\', filename=\'activity_inspector.js\') }}"></script>\n</head>',
                1
            )
            needs_update = True
        else:
            # Remove defer attribute if present to ensure it initializes correctly
            content = content.replace(
                '<script defer src="{{ url_for(\'static\', filename=\'activity_inspector.js\') }}">',
                '<script src="{{ url_for(\'static\', filename=\'activity_inspector.js\') }}">'
            )

        if needs_update:
            with open(template_path, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"  ✅ Updated Inspector includes in {template_path}")
        else:
            print(f"  ℹ️ Inspector already included in {template_path}")
...
for tpl in [
    ...
    verify_and_inject_inspector(tpl)


print("\n🎉 Activity Inspector Repair Applied!")
print("")
print("🔍 HOW TO TEST:")
print("  1. Restart Flask:  python app.py")
print("  2. Open browser to http://localhost:5000")
print("  3. Open browser DevTools (F12) → Console tab")
print("  4. You should see: [Activity Inspector] ✅ Ready.")
print("  5. Upload an XER file and click any activity code")
print("  6. Console will log: [Activity Inspector] Click #1 detected code: \"XXXXX\"")
print("  7. Drawer will slide in from the right")
print("")
print("🛠️ IF STILL NOT WORKING:")
print("  In the browser console, type:  window.__aiDiagnostics()")
print("  This will show you what's on the page and what was detected.")
print("")
print("💡 MANUAL TEST (in browser console):")
print("     window.__aiOpenActivity('A1000')")
print("  If this opens the drawer, the fetch works — but the click detection needs review.")